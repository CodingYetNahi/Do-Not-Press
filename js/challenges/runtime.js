...
function path(c,m){const cols=m.cols,rows=Math.ceil(m.n/cols),grid=el('div','path-grid');grid.style.gridTemplateColumns=`repeat(${cols},1fr)`;c.arena.append(grid);const cells=[];for(let i=0;i<m.n;i++){const b=el('button','path-cell','');b.type='button';grid.append(b);cells.push(b)}let base=[];for(let r=0;r<rows;r++){const start=r*cols,end=Math.min(start+cols,m.n),row=[];for(let x=start;x<end;x++)row.push(x);if(r%2)row.reverse();base.push(...row)}if(Math.random()<.5)base.reverse();const pathSeq=base,pulse=Math.max(180,Math.min(520,2200/m.n));pathSeq.forEach((idx,i)=>c.later(()=>{cells[idx].classList.add('lit');c.later(()=>cells[idx].classList.remove('lit'),pulse*.72)},300+i*pulse));const preview=450+m.n*pulse;c.later(()=>{c.sub.textContent='Repeat the path.';let k=0;cells.forEach((b,i)=>c.listen(b,'pointerdown',()=>{if(i!==pathSeq[k])return c.finish(false,'Path broken.');b.classList.add('picked');if(++k===pathSeq.length)c.finish(true)}))},preview);time(c,'Path expired.',m.limitMs)}
...
add('order',[false,true,false,true,false].map(rev=>({title:rev?'5 → 1.':'1 → 5.',sub:'Numbers move after every correct tap.',rev})));
